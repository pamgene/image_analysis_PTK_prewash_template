# PTK image analysis PreWash Template

- Version 0.2.3 and earlier versions: enrichment contains Array Layouts 86312, 86402, 86412, 87102
- Version 0.2.4: enrichment contains Array Layouts 86312, 86402, 86412, 87102, 87202
- Version 0.2.7: Bug of not saving replaced spot is resolved. Enrichment of UKAdb 0.5.0
- Version 0.2.8: shiny apps replaced to flutter

Prior to running, upload zip folder (contains: ImageResults folder, correct Array Layout) with upload file button. Don't delete gitattributes and gitignore.